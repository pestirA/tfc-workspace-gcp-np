...
/**role   = "roles/iam.serviceAccountUser"
role   = ["roles/iam.serviceAccountUser","roles/billing.user"]**/
/******************************************
  Granting serviceAccountUser to multiple roles
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_billingUser" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to organizationViewer
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_organizationViewer" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to organizationViewer
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_projectCreator" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to compute.xpnAdmin
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_computeXpnAdmin" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to compute.storageAdmin
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_storageAdmin" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to compute.storageAdmin
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_roleBrowser" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
  role   = "roles/browser"
  member = local.group_id
...
/******************************************
  Granting serviceAccountUser to roles/compute.networkAdmin
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_networkAdmin" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
...
/******************************************
  Granting serviceAccountUser to projectIamAdmin
 *****************************************/
resource "google_service_account_iam_member" "service_account_grant_to_projectIamAdmin" {
  count = var.manage_group && var.create_project_sa ? 1 : 0
